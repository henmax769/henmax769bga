...
LiquidCrystal_I2C lcd(0x27,2,1,0,4,5,6,7,3,POSITIVE); //вместо (0x3f) указываем адрес своего дисплея

//RelayPin "1"-ВЕРХНИЙ нагреватель
//RelayPin "2"-НИЖНИЙ нагреватель
#define RelayPin1 2  //назначаем пин "ВЕРХНЕГО" нагревателя
#define RelayPin2 47  //назначаем пин "НИЖНЕГО" нагревателя
int curCount = 0;
...
void loadProfile()//this function loads whichever profile currentProfile variable is set to
{
  ...
  for (int i = 0; i < 9; i + 1) {
    temperatureStep[i] = EEPROM.read((currentProfile - 1) * 29 + i + 20) * 5;
    i++;
  }
...
void setup()
{
  ...
  digitalWrite(editSwitchPin, HIGH); //подключаем подтягивающий резистор
  ...
  lcd.clear();
  //lcd.setCursor(1, 1);.......//.......
  lcd.print("REWORK STATION      by.hen MAX");
  //Welcome melody
  tone(buzzerPin, 523);
  delay(200);
  tone(buzzerPin, 659);
...
//-------------------------------------------------------------------------------
  ...
  switch (reflowState)
  {
    ...
    case REFLOW_STATE_MENU_STEP_TARGET:
      ...
      if (upSwitchState == HIGH)
      {
        temperatureStep[editStep] = temperatureStep[editStep] + 5;
        delay(25);
        if (temperatureStep[editStep] >= 400)
        {
          temperatureStep[editStep] = 400;
        }
      }
      if (downSwitchState == HIGH)
      {
    ...
    case REFLOW_STATE_MENU_TOP_D:
      ...
      if (okSwitchState == HIGH)
      {
        ...
        for (int i = 0; i < 9; i + 1) {
          EEPROM.write(((currentProfile - 1) * 29 + i + 20), temperatureStep[i] / 5);
          i++;
        }
    ...
    case REFLOW_STATE_STEP_RAMP:
      ...
      startTemp = tc1;// нужно как-то привязать к начальной температуре
